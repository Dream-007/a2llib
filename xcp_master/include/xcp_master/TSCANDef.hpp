...
typedef void(* TCANFDOnQueueEvent_WHandle)(size_t* obj,const TLibCANFD* AData);
#endif
...
extern "C" u32 tscan_register_event_canfd_whandle(const size_t ADeviceHandle, const TCANFDOnQueueEvent_WHandle ACallback);
//UnRegister CAN message received event
extern "C" u32 tscan_unregister_event_canfd_whandle(const size_t ADeviceHandle, const TCANFDOnQueueEvent_WHandle ACallback);
...
extern "C" u32 tscan_add_cyclic_msg_can_wo_compensation(const size_t ADeviceHandle, const PLibCAN AData, const float ATimeMs); 
extern "C" u32 tscan_add_cyclic_msg_canfd_wo_compensation(const size_t ADeviceHandle, const PLibCANFD AData, const float ATimeMs);
...
extern "C" u32 tscan_configure_can_regs(
	const size_t ADeviceHandle,
    const APP_CHANNEL AChnIdx,
    const double ABaudrateKbps,
    const u32 ASEG1,
	const u32 ASEG2,
	const u32 APrescaler,
	const u32 ASJW,
    const u32 AOnlyListen,
    const u32 A120OhmConnected,
    const s32 AIntervalCnt);

extern "C" u32 tscan_configure_canfd_regs(
      const size_t ADeviceHandle,
      const APP_CHANNEL AChnIdx,
      const double AArbBaudrate,
      const u32 AArbSEG1,
	  const u32 AArbSEG2,
	  const u32 AArbPrescaler,
	  const u32 AArbSJW,
      const double ADataBaudrate,
      const u32 ADataSEG1,
	  const u32 ADataSEG2,
	  const u32 ADataPrescaler,
	  const u32 ADataSJW,
      const TLIBCANFDControllerType AControllerType,
      const TLIBCANFDControllerMode AControllerMode,
      const u32 A120OhmConnected,
      const s32 AIntervalCnt);
